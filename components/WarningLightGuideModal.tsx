      ...
      <div className="max-h-[70vh] overflow-y-auto pr-2 -mr-2 scroll-smooth">
        ...
        <div className="space-y-4">
          {warningLights.map((light) => (
            <div key={light.name} className="bg-light-panel-muted/80 dark:bg-dark-panel-muted/50 p-4 rounded-lg border border-light-border dark:border-dark-border">
              <div className="flex items-start gap-4">
                <div className="flex-shrink-0">
                  <light.Icon className={`w-10 h-10 ${light.color}`} />
                </div>
                <div className="flex-1 min-w-0">
                  <h4 className="font-bold text-light-text dark:text-dark-text">{light.name}</h4>
                  <div className="mt-2">
                    <p className="text-xs font-semibold uppercase text-light-muted dark:text-dark-muted tracking-wider">What it means</p>
                    <p className="text-sm text-light-text dark:text-dark-text mt-1">{light.meaning}</p>
                  </div>
                  <div className="mt-3">
                    <p className="text-xs font-semibold uppercase text-danger tracking-wider">What to do</p>
                    <p className="text-sm text-light-text dark:text-dark-text mt-1">{light.action}</p>
                  </div>
                </div>
              </div>
            </div>
