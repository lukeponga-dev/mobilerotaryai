            ...
            {sortedSessions.map((session, index) => (
                <div 
                    ...
                    className={`
                        ...
                        transition-all duration-300 ease-in-out hover:bg-slate-200/60 dark:hover:bg-slate-700/60 hover:ring-2 hover:ring-orange-500/50
                        ${deletingId === session.id ? 'opacity-0 scale-95 -translate-x-8' : 'opacity-100'}
                    `}
                >
                    {index === 0 && (
                        <span className="absolute -top-2.5 right-3 text-xs font-bold bg-orange-500 text-white px-2.5 py-1 rounded-full shadow-md z-10 tracking-wider">
                            RECENT
                        </span>
                    )}
                    <a href={`#/session/${session.id}`} className="flex-1 min-w-0">
                        <p className="font-semibold text-slate-800 dark:text-slate-100 truncate group-hover:text-orange-500 transition-colors">{session.name}</p>
