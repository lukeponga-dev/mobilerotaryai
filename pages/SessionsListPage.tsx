  ...
  return (
    <div className="flex-1 flex flex-col min-w-0 bg-slate-900">
      ...
      <div className="flex-1 p-4 md:p-6 overflow-y-auto">
        <h1 className="text-3xl font-bold text-white mb-6">All Diagnosis Sessions</h1>
        {sortedSessions.length > 0 ? (
            <div className="space-y-3">
            {sortedSessions.map(session => (
                <div key={session.id} className="group bg-slate-800 rounded-lg border border-slate-700/50 flex items-center justify-between p-4 hover:bg-slate-700/50 transition-colors">
                    <a href={`#/session/${session.id}`} className="flex-1 min-w-0">
                        <p className="font-semibold text-slate-100 truncate group-hover:text-rose-400 transition-colors">{session.name}</p>
                    ...
                    <button
                        ...
                        className="text-slate-500 hover:text-rose-500 transition-colors ml-4 flex-shrink-0 p-2"
          ...
          <div className="text-center py-12">
            <h2 className="text-2xl font-semibold text-slate-300">No Sessions Found</h2>
            <p className="text-slate-500 mt-2 mb-6">Get started by creating a new diagnosis session.</p>
            <button
      ...
      >
        ...
        <div className="mt-6 flex justify-end gap-3">
          <button
            ...
            className="px-4 py-2 text-sm font-medium text-slate-200 bg-slate-600 rounded-lg hover:bg-slate-500 transition-colors"
          >
            Cancel
          </button>
          <button
